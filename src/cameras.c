...
static int freenect_fetch_zero_plane_info(freenect_device *dev)
{
	...
	int res;
	res = send_cmd(dev, 0x04, cmd, 10, reply, FREENECT_ZERO_PLANE_SIZE); //OPCODE_GET_FIXED_PARAMS = 4,
	if (res != FREENECT_ZERO_PLANE_SIZE) {
	FN_ERROR("freenect_fetch_zero_plane_info: send_cmd read %d bytes (expected %i)\n", res, FREENECT_ZERO_PLANE_SIZE);
